...
const BookAppointmentModal = ({ isOpen, onClose, onSuccess, initialData, title }) => {
  ...
  const loadDoctors = useCallback(async () => {
    try {
      ...
      const profileDoctorId = await resolveDoctorId(currentUser);
      const firstDoctorId = doctorList[0]?.doctorId ?? doctorList[0]?.DoctorId;

      const resolvedId = profileDoctorId || firstDoctorId || '';
      setFormData((prev) => ({
        ...prev,
        doctorId: String(prev.doctorId || resolvedId || ''),
      }));
      await syncSchedulesForDoctor(profileDoctorId || firstDoctorId);
    } catch (error) {
