...
import { CalendarDaysIcon, ClockIcon, PlusIcon, BanknotesIcon, CheckCircleIcon, ExclamationCircleIcon } from '@heroicons/react/24/outline';
...
const NurseAppointments = () => {
  ...
  const { invoicesState, servicesState, createInvoice, addPayment } = useBilling();
  ...
  // Look up the consultation service price from billing services
  const consultationService = useMemo(() => {
    return (servicesState.items || []).find(
      (s) => s.serviceName === 'Orthopedic Consultation' && s.activeStatus
    );
  }, [servicesState.items]);
  ...
  const getPaymentStatus = useCallback((appointmentId) => {
  ...
  const getInvoiceForAppointment = useCallback((appointmentId) => {
    return appointmentInvoices.get(String(appointmentId)) || null;
  }, [appointmentInvoices]);

  const handleConfirmAndInvoice = async (appointment) => {
    try {
      const service = consultationService;
      if (!service) {
        toast.error('Consultation service not found. Please configure service pricing.');
        return;
      }
      const patientId = appointment.patientId
        ? `PAT-${1000 + Number(appointment.patientId)}`
        : 'PAT-1001';
      ...
      await createInvoice({
        ...
        referenceId: String(appointment.appointmentId),
      });
      ...
      toast.success(`Invoice generated for ${patientName}. Awaiting payment.`);
    } catch (error) {
      toast.error('Failed to generate invoice');
    }
  };

  const handleRecordPayment = (appointment) => {
    const invoice = getInvoiceForAppointment(appointment.appointmentId);
    if (!invoice) {
  ...
  const handlePaymentSubmit = async ({ amount, method }) => {
    ...
    await addPayment({
      ...
      amount,
      method,
      patientId: paymentInvoice.patientId,
      patientName: paymentInvoice.patientName,
    });
    setPaymentInvoice(null);
    // Update local appointment status to Confirmed after payment
    const refId = paymentInvoice.referenceId;
    setAppointments((prev) =>
      prev.map((appt) =>
        String(appt.appointmentId) === String(refId)
              ...
              {filteredAppointments.map((appointment) => (
                <AppointmentCard key={appointment.appointmentId}>
                  <CardBody>
                    ...
                    {/* Service Price & Payment Status */}
                    <div style={{ marginTop: '12px', display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '12px' }}>
                      {consultationService && (
                        <PriceTag>
                          <BanknotesIcon />
                          {formatCurrency(consultationService.price)}
                        </PriceTag>
                      )}
                      <PaymentStatusBadge $status={getPaymentStatus(appointment.appointmentId)}>
                        {getPaymentStatus(appointment.appointmentId) === 'Paid' && <CheckCircleIcon />}
                        {getPaymentStatus(appointment.appointmentId) === 'Pending' && <ExclamationCircleIcon />}
                        {getPaymentStatus(appointment.appointmentId) === 'Payment Pending' && <ExclamationCircleIcon />}
                        {getPaymentStatus(appointment.appointmentId) === 'Not Invoiced' ? 'Not Invoiced' : getPaymentStatus(appointment.appointmentId) === 'Paid' ? 'Paid' : 'Payment Pending'}
                      </PaymentStatusBadge>
                    </div>
                    ...
                    <div style={{ marginTop: '16px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                      {appointment.status === 'Scheduled' && (
                        <>
                          <Button
                            size="small"
                            variant="primary"
                            onClick={() => handleConfirmAndInvoice(appointment)}
                      ...
                      {appointment.status === 'Payment Pending' && (
                        <>
                          <Button
                            size="small"
                            variant="success"
                            onClick={() => handleRecordPayment(appointment)}
                            style={{ display: 'flex', alignItems: 'center', gap: '6px' }}
                          >
                            <BanknotesIcon style={{ width: '16px', height: '16px' }} />
                            Record Payment
                      ...
                      {appointment.status === 'Confirmed' && (
                        <span style={{ fontSize: '13px', color: '#15803d', fontWeight: 600 }}>Confirmed - Payment Received</span>
                      )}
                    </div>
                  </CardBody>
                </AppointmentCard>
